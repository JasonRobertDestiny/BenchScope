...
│    - 论文库: arXiv, ACL, NeurIPS, ICLR, ICML                   │
│    - 评测榜单: HELM, Open LLM Leaderboard, EvalPlus           │
│    - 开源社区: GitHub Trending, HuggingFace Hub               │
│    - 团队线索: 飞书群聊、内部讨论                              │
│    - 社交媒体: Twitter/X (可选)                                │
└──────────────────────────────────────────────────────────────┘
                           ↓
...
**说明**: 以下字段支撑"一键添加"核心目标，确保研究团队能快速评估Benchmark价值

| 类别 | 字段名 | 飞书列名 | 数据类型 | 必需性 | 说明 |
|------|--------|----------|---------|-------|------|
| **基础信息** | title | 标题 | 文本 | ✅ 必需 | Benchmark名称 |
| | source | 来源 | 单选 | ✅ 必需 | arXiv/GitHub/HuggingFace/SemanticScholar |
| | url | URL | URL | ✅ 必需 | 主要链接（GitHub/论文页面） |
| | abstract | 摘要 | 文本 | ✅ 必需 | Benchmark简介（≤500字） |
| **关键资源** | paper_url | 论文 URL | URL | ⚠️ 强烈推荐 | 论文原文链接 |
| | dataset_url | 数据集 URL | URL | ⚠️ 强烈推荐 | 数据集下载/查看链接 |
| | reproduction_script_url | 复现脚本链接 | URL | 可选 | 评估/复现脚本仓库 |
| **元数据** | evaluation_metrics | 评价指标摘要 | 文本 | ⚠️ 强烈推荐 | 如"Accuracy, F1, BLEU" |
| | publish_date | 开源时间 | 日期 | 可选 | 首次发布日期 |
| | task_type | 任务类型 | 单选 | ⚠️ 强烈推荐 | Code Generation/QA/Reasoning等 |
| | license_type | License类型 | 文本 | 可选 | MIT/Apache-2.0/GPL等 |
| | github_stars | GitHub Stars | 数字 | 可选 | GitHub项目星标数 |
| | authors | 作者信息 | 文本 | 可选 | 主要作者列表（≤200字） |
| **评分维度** | activity_score | 活跃度 | 数字 | ✅ 必需 | 0-10分 |
| | reproducibility_score | 可复现性 | 数字 | ✅ 必需 | 0-10分 |
| | license_score | 许可合规 | 数字 | ✅ 必需 | 0-10分 |
| | novelty_score | 任务新颖性 | 数字 | ✅ 必需 | 0-10分 |
| | relevance_score | MGX适配度 | 数字 | ✅ 必需 | 0-10分 |
| | total_score | 总分 | 数字 | ✅ 必需 | 加权总分(0-10) |
| | priority | 优先级 | 单选 | ✅ 必需 | high/medium/low |
| | reasoning | 评分依据 | 文本 | ✅ 必需 | LLM解释为何作为候选（≤500字） |
| **状态管理** | status | 状态 | 单选 | ✅ 必需 | pending/approved/rejected |

**字段优先级说明**:
- ✅ **必需**: 系统自动填充，缺失会导致评分失败
- ⚠️ **强烈推荐**: 支撑快速决策的关键信息，缺失会降低候选质量
- **可选**: 锦上添花的补充信息
...
- 核心功能: arXiv/GitHub/HuggingFace/SemanticScholar采集 + URL去重 + LLM评分(GPT-4o) + 飞书存储/通知
